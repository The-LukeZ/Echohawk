...
- `commands.go` - the `/config` slash command tree (`commandDefs`), its handler (`Checker.HandleConfigCommand`), the message-template edit modal (`Checker.HandleConfigMessageModalSubmit`), and per-subcommand validation
...
- **Customizable messages**: the `messages` table holds one template per key (`alert`, `dm_user`, `timeout_reason`, `kick_reason`, `ban_reason`), seeded with defaults on first run. `renderMessage` (`actions.go`, a free function taking `*Config`) fills in `{user_id}`, `{count}`, `{window}`, `{content}` via `strings.NewReplacer`. Edit via `/config message set <key>` (opens a modal - see below), preview with `/config message get <key>` (ephemeral code-block embed), or restore the default with `/config message reset <key>`.
- **`/config` command**: single guild-scoped slash command with subcommands `view`/`set` and subcommand groups `actions` (add/remove), `excluded_channels` (add/remove), `message` (set/get/reset). Gated by `DefaultMemberPermissions = PermissionManageGuild` at Discord's level, re-checked in `HandleConfigCommand` (and again in `HandleConfigMessageModalSubmit`, since a modal submission is a separate interaction) against the invoking member. Every setter in `store.go` (`SetSimilarityMin`, `SetAlertAfter`, etc.) re-`Load()`s and returns a fresh `*Config`, which the handler then pushes via `checker.SetCfg`. Registered per-guild (instant propagation) on every startup in `main()` - idempotent, so command tree changes ship with each deploy.
- **`/config message set` modal**: rather than a slash-command string option (which can't take literal newlines or show the existing value), `set` opens a modal with a single paragraph `TextInputComponent` pre-filled with the current template. The chosen key round-trips through the modal's `custom_id` (`config_message_set:<key>`, see `messageModalCustomID`/`parseMessageModalCustomID`) since the modal itself can't carry extra state between the two separate interactions (opening it, then its submission). Submission is validated the same way as other message writes (non-empty, no unknown `{...}` placeholders) in `HandleConfigMessageModalSubmit`.
...
Full behavioral spec: `docs/02_HOW_IT_WORKS.md` (kept in sync with the SQLite config + `/config` command). `docs/01_SETUP.md` may still be stale re: the SQLite migration - verify against `db.go`/`store.go` before trusting env var claims there.
